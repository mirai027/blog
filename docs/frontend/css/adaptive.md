...
原理放到现在也很好理解。根据设备屏幕的DPR,自动设置最合适的高清缩放（body的字体大小，重置根目录的字体大小）。`width`=设备独立像素的宽度，初始/最大/最小缩放=1。通过 js计算`root fontSize`的值，使`1rem` = 设计稿 / 10。
...
[demo仓库地址](https://github.com/mirai027/miari-adaptive)
